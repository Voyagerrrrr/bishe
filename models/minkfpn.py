...
from layers.dropout import MinkowskiDropout
...
class MinkFPNwDropout(ResNetBase):
	# Feature Pyramid Network (FPN) architecture implementation using Minkowski ResNet building blocks
	def __init__(self, in_channels, out_channels, num_top_down=1, conv0_kernel_size=5, block=BasicBlock,
				 layers=(1, 1, 1), planes=(32, 64, 64), dropout_location = -1, dropout_rate = 0.0):
		assert len(layers) == len(planes)
		assert 1 <= len(layers)
		assert 0 <= num_top_down <= len(layers)
		self.num_bottom_up = len(layers)
	...
	def network_initialization(self, in_channels, out_channels, D):
		...
		self.relu = ME.MinkowskiReLU(inplace=True)
		self.dropout = MinkowskiDropout(self.dropout_rate)
	...
	def forward(self, x):
		...
		if self.dropout_location == -3:
			#are we currently training?
			training = self.training
			if not training:
				self.train() #dropout is only applied normally when a model is in 'train' mode
			x = self.dropout(x)
			if not training:
